...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Get user from the data base based on the user email
        user = User.query.filter_by(email=u_email).first()
        # Email doesnt exist or password incorrect
        if not user:
            flash("Email doesn't exist, please try again or register to continue")
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to log in to register your comment")
            return redirect(url_for("login"))
        else:
